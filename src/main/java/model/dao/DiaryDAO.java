...
private JDBCUtil jdbcUtil = null;
	...
	public List<Diary> findMonthlyDiary(long memberId, String... date) throws SQLException {
		...
		if (date.length == 2) {
			...
			jdbcUtil.setSqlAndParameters(sql, new Object[] {memberId, date[0], date[1]});	// JDBCUtil에 query문과 매개 변수 설정
		}
		...
		if (date.length == 1) {
			...
			jdbcUtil.setSqlAndParameters(sql, new Object[] {memberId, "YYMMDD", date[0]});	// JDBCUtil에 query문과 매개 변수 설정
		}
			
		// diary id를 기준으로 drinkingList에 정보 저장 -> diary.setDrinkingList() 호출 -> diaryList에 diary 저장
	...
	public Diary createDiary(Diary diary) throws SQLException { // 추가한 diary 객체 반환
		...
		try { 
			...
			Object[] param1 = new Object[] {diary.getMember().getId(),
							diary.getDrinkingDate(), diary.getCondition(), diary.getContent()};				
			...
			jdbcUtil.executeUpdate(key1);  // insert 문 실행
		   	...
		   	for (Drink dr : diary.getDrinkingList()) {
				...
				if (preferenceId == -1) {	// 테이블 존재 X
					rcd.createPreferenceByAmount(diary.getMember().getId(), dr.getAlcohol().getAlcoholId(), dr.getAmount());
				} else {
